"""ADK 2.0 Workflow Graph for Say [피사체 유형].
...
# Wrap functions into FunctionNodes
node_safety = FunctionNode(func=safety_guardrail_func, name="safety_guardrail")
node_transform = FunctionNode(func=nanobanana_transform_func, name="nanobanana_transform")
node_format = FunctionNode(func=format_output_func, name="format_output")

# Define the ADK 2.0 Workflow Graph Edges
edges = [
    Edge(from_node=START, to_node=node_safety),
    Edge(from_node=node_safety, to_node=node_transform, route="safe"),
    Edge(from_node=node_safety, to_node=node_format, route="blocked"),
    Edge(from_node=node_transform, to_node=node_format, route="done"),
]

# Define the ADK 2.0 Workflow Graph
say_workflow = Workflow(
    name="say_subject_transformer",
    description="Transforms photo main subject into requested material type after passing safety moderation.",
...
adk_app = App(name="say_app", root_agent=say_workflow)
adk_runner = InMemoryRunner(app=adk_app)


async def execute_say_pipeline(image_b64: str, target_keyword: str) -> TransformResponse:
