"""NanoBanana Image Transformation Service for Say [피사체 유형].

Transforms the main subject in a user's photo into a target material/style (e.g. Cheese, Stone, Candy)
using Gemini Multimodal Image Model (gemini-3.1-flash-lite-image).
"""
...
def transform_subject_image(image_base64: str, target_keyword: str) -> dict:
    ...
    Returns:
        dict: {'success': bool, 'transformed_image': str (base64 data URL), 'description': str}
    """
    ...
    if gemini_key:
        try:
            ...
            prompt = (
                f"Transform the main subject in this photograph into {target_keyword} material/style. "
                f"Render the subject crafted out of {target_keyword} with realistic surface texture, color, and lighting, "
                f"while maintaining the overall composition."
            )
            ...
            if response.candidates and response.candidates[0].content and response.candidates[0].content.parts:
                for part in response.candidates[0].content.parts:
                    if part.inline_data:
                        gen_b64 = base64.b64encode(part.inline_data.data).decode("utf-8")
                        mime = part.inline_data.mime_type or "image/jpeg"
                        return {
                            "success": True,
                            "transformed_image": f"data:{mime};base64,{gen_b64}",
                            "description": f"피사체가 Gemini AI (gemini-3.1-flash-lite-image) 모델을 통해 '{target_keyword}' 형태로 성공적으로 변환되었습니다!"
                        }
    ...
    # Applies artistic material textures, color palettes, and highlights matching target_keyword
    ...
    return {
        ...
        "description": f"피사체가 '{target_keyword}' 재질 및 텍스처로 변환되었습니다 (NanoBanana Visual Engine)."
    }


def apply_material_stylization(img: Image.Image, keyword: str) -> Image.Image:
    """Applies stylized material textures and thematic effects to the image based on keyword."""
    w, h = img.size
    ...
    margin_w = int(w * 0.15)
    margin_h = int(h * 0.15)
    draw_mask.ellipse([margin_w, margin_h, w - margin_w, h - margin_h], fill=220)
    ...
    if any(k in keyword_lower for k in ["치즈", "cheese"]):
        ...
        for _ in range(25):
            ...
            draw.ellipse([cx-r, cy-r, cx+r, cy+r], fill=(210, 150, 0, 180), outline=(255, 230, 100))
