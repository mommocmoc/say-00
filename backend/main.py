...
from backend.workflow import execute_say_pipeline, TransformRequest, TransformResponse
...
async def transform_endpoint(payload: TransformRequest):
    """Processes image subject transformation through ADK 2.0 Graph Workflow."""
    if not payload.image_b64 or not payload.target_keyword:
        raise HTTPException(status_code=400, detail="image_b64 and target_keyword are required.")
